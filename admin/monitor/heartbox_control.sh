...
manage_wireguard() {
    ...
    case $wg_choice in
        1) 
            if [[ -x "${MY_PATH}/../swarm/wireguard_control.sh" ]]; then
                # On lance le sous-menu complet du serveur
                "${MY_PATH}/../swarm/wireguard_control.sh"
            else
            ...
            ;;
        2)
            if [[ -x "${MY_PATH}/../swarm/wg-client-setup.sh" ]]; then
                # On lance le script de configuration interactif client
                "${MY_PATH}/../swarm/wg-client-setup.sh"
            else
        ...
        3)
            if [[ -x "${MY_PATH}/../swarm/wg-client-setup.sh" ]]; then
                ...
                "${MY_PATH}/../swarm/wg-client-setup.sh" qr
            else
...
manage_swarm() {
    ...
    case $swarm_choice in
        ...
        2) 
            ...
            ;;
        3) "${MY_PATH}/../swarm/SWARM.notifications.sh" ;;
        4) "${MY_PATH}/../ZEN.SWARM.payments.sh" ;;
        5) "${MY_PATH}/../swarm/SWARM.help.sh" ;;
    esac
