...
coop_publish_config_to_nostr() {
    ...
    # SECURITY CHECK: Ensure no sensitive keys are in plain text
    local sensitive_keys="TOKEN SECRET KEY PASSWORD API PRIVATE"
    local validation_failed=0
    
    # Iterate over all keys in the JSON
    for key in $(echo "$config_json" | jq -r 'keys[]' 2>/dev/null); do
    ...
    if [[ $validation_failed -eq 1 ]]; then
        echo "[ERROR] Publication blocked due to security violations." >&2
        return 1
    fi
...
# Export decrypted config to a .env file
# Usage: coop_export_env ".env"
coop_export_env() {
    local output_file="$1"
    if [[ -z "$output_file" ]]; then
        echo "[ERROR] Usage: coop_export_env <output_file>" >&2
        return 1
    fi
    
    local config=$(coop_load_config)
    if [[ -z "$config" ]] || [[ "$config" == "{}" ]]; then
        echo "[ERROR] No config to export" >&2
        return 1
    fi
    
    # Create/overwrite file with header
    echo "# Generated by cooperative_config.sh on $(date)" > "$output_file"
    echo "# CAUTION: This file contains decrypted secrets. Do not commit to git!" >> "$output_file"
    echo "" >> "$output_file"
    
    # Iterate keys and decrypt values
    # We use a temporary file to handle the loop output correctly
    local temp_env=$(mktemp)
    
    echo "$config" | jq -r 'keys[]' 2>/dev/null | while read key; do
        # Skip comments and metadata
        if [[ "$key" == _* ]] || [[ "$key" == "COOPERATIVE_VERSION" ]] || [[ "$key" == "CREATED_AT" ]]; then
            continue
        fi
        
        # Get decrypted value
        local val=$(coop_config_get "$key")
    ...
    cat "$temp_env" >> "$output_file"
    rm "$temp_env"
    
    echo "[OK] Configuration exported to $output_file"
}
