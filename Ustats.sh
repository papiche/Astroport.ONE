...
if [[ ! -s ~/.zen/tmp/${CACHE_FILE} ]]; then
    ...
    if [[ -n "$ULAT" && -n "$ULON" ]]; then
        final_json="{\"version\" : \"54321.0\", \"DATE\": \"$(date -u)\", \"uSPOT\": \"$uSPOT\", \"g1station\": \"$myLIBRA/$IPFSNODEID\", \"PAF\": \"$PAF\", \"NCARD\": \"$NCARD\", \"ZCARD\": \"$ZCARD\", \"myRELAY\": \"$myRELAY\", \"IPFSNODEID\": \"$IPFSNODEID\", \"myIPFS\": \"${myIPFS}\", \"UPLANETG1PUB\": \"$UPLANETG1PUB\", \"G1\": \"$COINS\", \"ZEN\": \"$ZEN\", \"BILAN\": \"$BILAN\", \"CENTER\": {\"LAT\": \"$ULAT\", \"LON\": \"$ULON\", \"DEG\": \"$DEG\"}, \"CLOSEST_UMAPs\": [$closest_umaps_json_array], \"SWARM\": [$swarm_json_array], \"NOSTR\": [$nostr_json_array], \"PLAYERs\": [$tw_json_array], \"UMAPs\": [$umap_array_str]}"
    else
        final_json="{\"version\" : \"54321.0\", \"DATE\": \"$(date -u)\", \"uSPOT\": \"$uSPOT\", \"g1station\": \"$myLIBRA/$IPFSNODEID\", \"PAF\": \"$PAF\", \"NCARD\": \"$NCARD\", \"ZCARD\": \"$ZCARD\", \"myRELAY\": \"$myRELAY\", \"IPFSNODEID\": \"$IPFSNODEID\", \"myIPFS\": \"${myIPFS}\", \"UPLANETG1PUB\": \"$UPLANETG1PUB\", \"G1\": \"$COINS\", \"ZEN\": \"$ZEN\", \"BILAN\": \"$BILAN\", \"SWARM\": [$swarm_json_array], \"NOSTR\": [$nostr_json_array], \"PLAYERs\": [$tw_json_array], \"UMAPs\": [$umap_array_str]}"
    fi
