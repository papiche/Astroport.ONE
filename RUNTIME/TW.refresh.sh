...
################################################################################
#~ TW.refresh.sh
#~ Refresh TiddlyWiki data & synchronization
################################################################################
# Ce script gère la synchronisation et la gestion des TiddlyWiki :
# 1. Synchronisation IPNS des TW
# 2. Validation de l'intégrité des TW
# 3. Gestion des amis et relations
# 4. Export RSS et publication
# 5. Intégration géographique UMAP
################################################################################
...
if [[ -z "$PLAYER" || -z "$MOATS" || -z "$TW_FILE" ]]; then
    echo "Usage: $0 PLAYER MOATS TW_FILE"
    exit 1
fi

echo "## REFRESHING TW FOR ${PLAYER}"
...
[[ ! ${ASTRONAUTENS} ]] && echo "ERROR BAD ${PLAYER} - CONTINUE" && exit 1
...
echo "TW refresh completed for ${PLAYER}"
