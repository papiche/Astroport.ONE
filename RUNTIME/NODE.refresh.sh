...
exec 200>"/tmp/node_refresh.lock"
flock -n 200 || { echo "NODE.refresh.sh déjà en cours — exit"; exit 0; }
################################################################################
## MAP REFRESH
...
########################################################
echo "## CLEAN NOSTR STRANGERS / DISAPPEARED NODEs"
"${MY_PATH}/../admin/system/purge_nostr_strangers.sh" --clean 2>/dev/null
