...
if [[ -z $NPUB || -z $HEX || ! -d ~/.zen/game/nostr/$PLAYER || ! -s ~/.zen/game/nostr/$PLAYER/G1PUBNOSTR ]]; then
    ...
    ${MY_PATH}/../tools/mailjet.sh --template "$0" --expire 48h "$PLAYER" $RESTRICTION_EMAIL "🚫 MULTIPASS Requis - Création ZEN Card Bloquée"
...
####tools##########################################setUMAP_ENV.sh
eval "$(${MY_PATH}/../tools/setUMAP_ENV.sh "${LAT}" "${LON}" | tail -n 1)"
