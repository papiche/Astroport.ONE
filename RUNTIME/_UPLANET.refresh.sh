...
for UMAP in ${unique_combined[@]}; do
    ...
    #~ [[ ${COINS} == "" || ${COINS} == "null" ]] \
        ...
        #~ && ${MY_PATH}/../tools/PAYforSURE.sh "${HOME}/.zen/tmp/${MOATS}/${MOATS}.key" "1" "${UMAPG1PUB}" "UPLANET${UPLANETG1PUB:0:8}:INIT:${UMAP}" \
    ...
    UMAPGEN="/ipns/copylaradio.com/Umap.html?southWestLat=${LAT}&southWestLon=${LON}&deg=0.01&ipns=${TODATENS}/_index.html"
    USATGEN="/ipns/copylaradio.com/Usat.html?southWestLat=${LAT}&southWestLon=${LON}&deg=0.01&ipns=${TODATENS}/_index.html"
...
###########################################################################################
    ...
    | sed -e "s~_ZONE_~${UMAP}~g" \
              ...
              -e "s~_UPLANETLINK_~/ipns/copylaradio.com/map_render.html\?southWestLat=${LAT}\&southWestLon=${LON}\&deg=0.01~g" \
              -e "s~http://127.0.0.1:8080~~g" \
...
${MY_PATH}/_SECTOR.refresh.sh "${unique_combined[@]}"

######################################################
### LEVEL 3 ###########################################
######################################################
 ### REGION = 1° Planet Slice
${MY_PATH}/_REGION.refresh.sh  "${unique_combined[@]}"
######################################################
