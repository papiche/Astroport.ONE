...
if [[ -f "$_COOP_SH" ]]; then
    [[ -z "$GIT_HOST"  ]] && GIT_HOST=$(timeout 8 bash  "$_COOP_SH" get GIT_HOST  2>/dev/null | head -1 || true)
    [[ -z "$GIT_TOKEN" ]] && GIT_TOKEN=$(timeout 8 bash "$_COOP_SH" get GIT_TOKEN 2>/dev/null | head -1 || true)
    [[ -z "$GIT_OWNER" ]] && GIT_OWNER=$(timeout 8 bash "$_COOP_SH" get GIT_OWNER 2>/dev/null | head -1 || true)
fi
