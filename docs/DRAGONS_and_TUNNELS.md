...
t+50m   ├─ REDÉMARRAGE SERVICES ────────────────────────────────
        ...
        │
t+52m   ├─ 🔍 WATCHDOG TUNNELS PERSISTANTS ──────────────────────
        │  Pour chaque ~/.zen/tunnels/enabled/x_*.sh :
        │    ipfs p2p ls | grep slug → actif ?
        │    NON → bash x_SLUG.sh (relance) >> tunnel.log &
        │
...
# Double tunnel : localhost + docker0 bridge (IP détectée dynamiquement)
