...
* `--no-ipfs`: Deprecated (IPFS upload removed for UPlanet\_FILE\_CONTRACT.md compliance)
...
* **Backward compatible**: Clients support both v1.0 (snake\_case, flat) and v2.0 (camelCase, nested)

📖 **See**: [INFO\_JSON\_FORMATS.md](../reference/INFO_JSON_FORMATS.md) for complete specification
...
**Note**: The main file CID is reused, but a **new info.json is created** with updated timestamp and upload\_chain. This allows the upload history to evolve with each re-publication.
...
✅ **Living History**: Each re-publication creates new info.json with updated timestamp and upload\_chain\
...
* `latitude`: Separate latitude tag (FLOAT\_STRING)
* `longitude`: Separate longitude tag (FLOAT\_STRING)
