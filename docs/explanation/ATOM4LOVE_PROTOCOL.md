...
Le système ATOM4LOVE repose sur une idée centrale : les paramètres physiques d'une personne — date, heure et lieu de naissance, poids, polarité biologique — peuvent être traduits en une **signature vibratoire** unique, objectivement calculable, et intégrable dans un protocole décentralisé via NOSTR.

Ce n'est pas de l'ésotérisme : c'est de la cryptographie douce. La "phase personnelle" φ est une valeur déterministe en radians, calculée par le moteur `Phi2X` (synchronisé entre `phi2x.js`, `phi2x.py`, et `Phi2X_Math.gd`). Le "taux de cohérence k" entre deux personnes est une mesure mathématique de l'alignement de leurs ondes.
...
φ\_i ∈ \[0, 2π) ≈ \[0, 6.28) — toujours < 7 (condition du filtre strfry)

### Fréquence biologique ω\_bio
...
2. Pour chaque profil reçu : calcul k = 1/(1+|sin(φ\_i − φ\_j)|) vs phase utilisateur
3. Affichage top-5 profils les plus résonants avec lien `minelife.html?npub=...`

L'onglet s'active via le listener `shown.bs.tab` → `loadRadar()` / `_queryRelayFilter()`. Les niveaux de résonance affichés : k ≥ 0.95 = 🔮 Singularité, k ≥ 0.85 = ✨ Super-cohérence, k ≥ 0.75 = 💫 Cohérence, k < 0.75 = 〰 Interférence.

Cette fonctionnalité est la passerelle web vers le réseau de résonance construit par cabine-33 dans le monde physique.

***

## Références

* [atomic.html](https://github.com/papiche/Astroport.ONE/blob/master/UPlanet/earth/atomic.html) — Interface web : PROFIL, Match, Théorie, Radar (Kind 30078)
* [phi2x.js](https://github.com/papiche/Astroport.ONE/blob/master/UPlanet/earth/phi2x.js) — Moteur Phi2X canonique (JS)
* [minelife.tuto.html](https://github.com/papiche/Astroport.ONE/blob/master/UPlanet/earth/minelife.tuto.html) — Affiche φ/ω des comptes via Kind 30078
* [NIP-101/filter/30078.sh](https://github.com/papiche/Astroport.ONE/blob/master/NIP-101/relay.writePolicy.plugin/filter/30078.sh) — Validation Certificat d'Incarnation
* [NIP-101/filter/30508.sh](https://github.com/papiche/Astroport.ONE/blob/master/NIP-101/relay.writePolicy.plugin/filter/30508.sh) — Validation Match vibratoire (k ≥ 0.85)
* [cabine-33/CLAUDE.md](https://github.com/papiche/Astroport.ONE/blob/master/cabine-33/CLAUDE.md) — Architecture app mobile Godot (producteur Kind 30508)
* [Astroport.ONE/tools/emit\_skill.sh](https://github.com/papiche/Astroport.ONE/blob/master/Astroport.ONE/tools/emit_skill.sh) — Kind 30503 avec expiration Tzolkin (260/780/2600 j)
* [Astroport.ONE/tools/cooperative\_config.sh](https://github.com/papiche/Astroport.ONE/blob/master/Astroport.ONE/tools/cooperative_config.sh) — Gestion AUTHORIZED\_APPS
* [NIP-101/backfill\_constellation.sh](https://github.com/papiche/Astroport.ONE/blob/master/NIP-101/backfill_constellation.sh) — Synchronise Kind 30078 + 30508 inter-relays
* [WOTX2\_SYSTEM.md](../reference/WOTX2_SYSTEM.md) — Architecture WoTx²
* [KIND\_30506\_JUSTICE.md](../reference/KIND_30506_JUSTICE.md) — Médiation
