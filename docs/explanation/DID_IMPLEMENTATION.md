...
Une **primo-transaction** est effectuée pour activer le compte ẐEN à 0 avec un virement de 1 Ğ1 depuis 🏛️ Réserve Ğ1 (UPLANETNAME\_G1).

**Caractéristiques** :

* **Montant** : 1 Ğ1 (activation initiale)
* **Source** : UPLANETNAME\_G1 (réserve centrale UPlanet)
...
1. **Détection** : [`primal_wallet_control.sh`](https://github.com/papiche/Astroport.ONE/blob/master/docs/explanation/tools/primal_wallet_control.sh) surveille les transactions entrantes
...
### 5.1. Génération de l'Identité (make\_NOSTRCARD.sh)
...
**Types de Jetons (`tokenTypes`)** : Le champ `tokenTypes` distingue les différents types de jetons possédés par l'utilisateur :
...
**Préservation du Statut de Sociétaire** : Lors d'une mise à jour MULTIPASS (recharge ZENCOIN), le système préserve automatiquement :

* Le `contractStatus` de sociétaire (cooperative\_member\_satellite, cooperative\_member\_constellation, infrastructure\_contributor)
...
✅ **Sécurité Distribuée** : Aucun point de défaillance unique ✅ **Contrôle Souverain** : L'utilisateur garde un pouvoir total sur sa relation de confiance ✅ **Interopérabilité** : Intégration fluide entre Ğ1 et services décentralisés ✅ **Réversibilité** : Une relation de confiance peut être rompue facilement ✅ **Potentiel Économique** : Location, sous-location, revenus passifs
...
📄 **Code source** : [`make_NOSTRCARD.sh` (lignes 246-345)](https://github.com/papiche/Astroport.ONE/blob/master/docs/tools/make_NOSTRCARD.sh#L246-L345)
...
* [Shamir Secret Sharing](https://en.wikipedia.org/wiki/Shamir's_Secret_Sharing) - Partage de secret cryptographique
...
* [MULTIPASS System Documentation](https://github.com/papiche/Astroport.ONE/blob/master/docs/explanation/MULTIPASS_SYSTEM.md) - Documentation complète du système MULTIPASS
* [MULTIPASS Quick Reference](https://github.com/papiche/Astroport.ONE/blob/master/docs/templates/MULTIPASS_QUICK_REFERENCE.md) - Guide rapide utilisateur
...
_"Dans UPlanet, votre identité vous appartient. Votre propriété numérique vous appartient. Vos données vous appartiennent. C'est plus qu'une promesse technique, c'est une promesse de liberté."_
