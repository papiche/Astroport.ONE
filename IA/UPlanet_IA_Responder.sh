    ...
    if [[ $KNAME =~ ^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$ || $KNAME == "CAPTAIN" ]]; then
        ...
        if [[ -z "$KeyANSWER" ]]; then
            if [[ "$message_text" =~ \#search ]]; then
                ...
                KeyANSWER="$($MY_PATH/question.py "MESSAGE: ${cleaned_text} # FOLLOW THE INSTRUCTIONS ! Reply in the same language as the message. # IMPORTANT ! Do NOT use markdown nor **bold** ! Use emojis to make your message more readable." --pubkey ${PUBKEY})"
                ################################################"
            fi
        fi
