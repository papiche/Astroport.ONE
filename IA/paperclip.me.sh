#!/bin/bash
################################################################################
## Paperclip AI Agents Swarm Connector
## Checks for local Paperclip on port 3100
...
SSH_OPTIONS="-fN -L 127.0.0.1:$PAPERCLIP_PORT:127.0.0.1:$PAPERCLIP_PORT"
...
print_header() {
    echo -e "${BOLD}${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}Paperclip AI Agents Connection Manager${NC}  (port $PAPERCLIP_PORT) ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
}
...
save_connection_status() {
    ...
    echo "CONNECTION_PORT=$PAPERCLIP_PORT" >> "$STATUS_FILE"
}

check_port() {
    local silent="${1:-false}"
    ss -tln 2>/dev/null | grep -qw ":$PAPERCLIP_PORT" && {
        [[ "$silent" != "true" ]] && echo "Port $PAPERCLIP_PORT is open."; return 0
    }
    [[ "$silent" != "true" ]] && echo "Port $PAPERCLIP_PORT is not available."; return 1
}

test_api() {
    local silent="${1:-false}"
    [[ "$silent" != "true" ]] && echo "Testing Paperclip API on port $PAPERCLIP_PORT..."
    RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:$PAPERCLIP_PORT/" --connect-timeout 5)
    if [[ "$RESPONSE" == "200" || "$RESPONSE" == "302" ]]; then
        [[ "$silent" != "true" ]] && echo "Paperclip responding correctly."
        return 0
    else
        [[ "$silent" != "true" ]] && echo "Paperclip not responding (HTTP $RESPONSE)."
        return 1
    fi
}
...
check_local_service() {
    local silent="${1:-false}"
    if docker ps 2>/dev/null | grep -q "paperclip" || \
       pgrep -f "paperclip" >/dev/null 2>&1; then
        [[ "$silent" != "true" ]] && print_status "OK" "Local Paperclip service detected"
        return 0
    fi
    return 1
}
...
check_ssh_tunnel_active() {
    local silent="${1:-false}"
    local pid=$(lsof -t -i :$PAPERCLIP_PORT 2>/dev/null)
...
connect_via_swarm() {
    ...
    if bash "$selected_script" 2>/dev/null; then
        ...
        print_status "FAIL" "Connected but Paperclip not responding"
    else
...
case "${1^^}" in
    "STATUS"|"ST")
        ...
        if check_port "true" && test_api "true"; then
            active_type="UNKNOWN"
            check_ssh_tunnel_active "true" && active_type="SSH"
            check_p2p_connections "true" && active_type="P2P"
            check_local_service "true" && active_type="LOCAL"
            echo -e "\n  ${GREEN}●${NC} ${BOLD}CONNECTED${NC} via ${CYAN}$active_type${NC}"
            echo -e "    └─ UI: ${GREEN}http://localhost:$PAPERCLIP_PORT${NC}"
        else
            echo -e "\n  ${RED}●${NC} ${BOLD}DISCONNECTED${NC}"
            echo -e "    └─ No active Paperclip connection"
        fi ;;
    "SCAN"|"DETECT"|"LIST")
        print_header; echo -e "\n${BOLD}Scanning...${NC}\n"
        check_local_service "true" && print_status "OK" "Local Paperclip" || print_status "FAIL" "No local Paperclip"
    ...
    "LOCAL")
        ...
        if check_local_service && check_port "true" && test_api "true"; then
            ...
            print_status "OK" "Connected to local Paperclip"
            echo -e "UI ready at ${GREEN}http://localhost:$PAPERCLIP_PORT${NC}"; exit 0
        fi
        print_status "FAIL" "Local Paperclip not available"; exit 1 ;;
    "SSH")   print_header; establish_ssh_tunnel "auto" && echo -e "UI: http://localhost:$PAPERCLIP_PORT" && exit 0; exit 1 ;;
    "SSH6")  print_header; establish_ssh_tunnel "ipv6" && exit 0; exit 1 ;;
    "SSH4")  print_header; establish_ssh_tunnel "ipv4" && exit 0; exit 1 ;;
    "P2P"|"SWARM"|"IPFS")
        print_header
        connect_via_swarm "$2" && echo -e "UI: http://localhost:$PAPERCLIP_PORT" && exit 0; exit 1 ;;
    "")
        if check_port "true" && test_api "true"; then
            echo "Paperclip ready at http://localhost:$PAPERCLIP_PORT"; exit 0
        fi
        if establish_ssh_tunnel "auto"; then
            echo "Paperclip ready via SSH at http://localhost:$PAPERCLIP_PORT"; exit 0
        fi
        echo "SSH unavailable, trying IPFS P2P swarm..."
        if connect_via_swarm; then
            echo "Paperclip ready via P2P at http://localhost:$PAPERCLIP_PORT"; exit 0
        fi
        echo "Could not connect to any Paperclip instance."; exit 1 ;;
    *)
