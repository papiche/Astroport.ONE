...
check_port() {
    if netstat -tulnp 2>/dev/null | grep ":$OLLAMA_PORT " >/dev/null; then
        echo "Port $OLLAMA_PORT is already open."
        return 0
    else
        echo "Port $OLLAMA_PORT is not open."
        return 1
    fi
}
...
test_api() {
    echo "Testing Ollama API connection on port $OLLAMA_PORT..."
...
close_ssh_tunnel() {
    ...
    PID=$(lsof -t -i :$OLLAMA_PORT 2>/dev/null)
    if [ -n "$PID" ]; then
        kill $PID 2>/dev/null
...
case "$1" in
    ...
    *)
        ...
        if check_port && test_api; then
            echo "Ollama API ready at http://localhost:$OLLAMA_PORT"
            exit 0
        fi
        
        # Try SSH tunnel to scorpio first (demo mode)
        if establish_ssh_tunnel; then
            echo "Ollama API ready via SSH tunnel at http://localhost:$OLLAMA_PORT"
            exit 0
        fi
        ...
        if connect_via_swarm; then
            echo "Ollama API ready via IPFS P2P at http://localhost:$OLLAMA_PORT"
            exit 0
        fi
        
        echo "Could not establish connection to any Ollama service."
        exit 1
        ;;
esac
