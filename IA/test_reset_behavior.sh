...
mkdir -p "$HOME/.zen/flashmem/$TEST_USER"

# Create test slot files
for i in {0..12}; do
    echo '{"user_id":"'$TEST_USER'","slot":'$i',"messages":[{"timestamp":"2024-01-01T12:00:00Z","content":"Test message for slot '$i'"}]}' > "$HOME/.zen/flashmem/$TEST_USER/slot$i.json"
done

echo "1. Created test slot files (0-12)"
ls -la "$HOME/.zen/flashmem/$TEST_USER/"

echo ""
echo "2. Testing reset specific slot (slot 3)..."
# Simulate #reset #3
rm -f "$HOME/.zen/flashmem/$TEST_USER/slot3.json"
...
# Simulate #reset #all
rm -f "$HOME/.zen/flashmem/$TEST_USER"/slot*.json
...
echo '{"user_id":"'$TEST_USER'","slot":0,"messages":[{"timestamp":"2024-01-01T12:00:00Z","content":"Test message for slot 0"}]}' > "$HOME/.zen/flashmem/$TEST_USER/slot0.json"
echo "Created slot 0 again"
ls -la "$HOME/.zen/flashmem/$TEST_USER/"

# Simulate #reset (should reset only slot 0)
rm -f "$HOME/.zen/flashmem/$TEST_USER/slot0.json"
