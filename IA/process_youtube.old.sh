...
EOF
    ...
    # Simplified retry logic - fewer attempts with simpler configs
    local max_retries=3
    local retry_count=0
    ...
    while [[ $retry_count -lt $max_retries && $download_success == false ]]; do
        if [[ $retry_count -gt 0 ]]; then
            local backoff_time=$((5 * retry_count))
        ...
        # Simplified client selection - try minimal configs first
        local client_selection=""
        case $retry_count in
            0) 
                # Strategy 1: No client specified - use yt-dlp defaults
                client_selection=""
                strategy="default"
                ;;
            1) 
                # Strategy 2: Simple client
                client_selection="youtube:player_client=android"
                strategy="android"
                ;;
            2) 
                # Strategy 3: TV client
                client_selection="youtube:player_client=tv_embedded"
                strategy="tv_embedded"
                ;;
        esac
