...
send_multipass_migration_notification() {
    ...
    if "${MY_PATH}/tools/mailjet.sh" --template "${MY_PATH}/templates/MULTIPASS/migration_notification.html" --expire 7d "$email" "$notification_file" "$subject" >/dev/null 2>&1; then
...
EOF
    ...
    "${MY_PATH}/tools/mailjet.sh" --template "$0" --expire 48h "$CAPTAINEMAIL" "$alert_file" "🚨 UPLANET Destruction - $wallet_name ($status)" >/dev/null 2>&1
