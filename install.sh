...
for i in pip python-dotenv setuptools wheel termcolor amzqr pdf2docx requests pyppeteer cryptography jwcrypto Ed25519 gql base58 pybase64 google silkaj pynacl python-gnupg pgpy pynentry paho-mqtt aiohttp ipfshttpclient bitcoin monero ecdsa pynostr bech32 duniterpy; do
        echo ">>> Installation $i <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
