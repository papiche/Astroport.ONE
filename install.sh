...
echo "#############################################"
for i in git zip ssss make cmake docker.io docker-compose hdparm iptables ufw fail2ban wireguard openssh-server parallel npm shellcheck multitail netcat-traditional ncdu chromium miller inotify-tools curl net-tools mosquitto libsodium* libcurl4-openssl-dev libgpgme-dev libffi-dev; do
